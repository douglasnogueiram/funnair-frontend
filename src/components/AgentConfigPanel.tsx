...
export default function AgentConfigPanel() {
  ...
  if (loading) return <div className="agent-config__loading">Carregando configuração…</div>

  return (
    <div className="agent-config">
      <div className="agent-config__header">
        <div>
          <h2 className="agent-config__title">Configuração do Agente</h2>
          <p className="agent-config__subtitle">
            Edite o prompt do sistema. Cada salvamento cria uma nova versão. Alterações têm efeito imediato.
          </p>
        </div>
        ...
        {isDirty && (
          <div className="agent-config__desc-row">
            <input
              ...
              placeholder="Descreva brevemente a alteração (opcional)"
              value={description}
      ...
      <section className="agent-config__history">
        <h3 className="agent-config__history-title">Histórico de versões</h3>
        {history.length === 0 ? (
          <p className="agent-config__empty">Nenhuma versão salva ainda.</p>
        ) : (
            ...
            {history.map(p => (
              <div key={p.id} className={`ac-version${p.active ? ' ac-version--active' : ''}`}>
                ...
                {!p.active && (
                  ...
                  >
                    {activatingId === p.id ? 'Ativando…' : 'Restaurar esta versão'}
                  </button>
                )}
              </div>
            ))}
