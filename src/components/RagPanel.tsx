...
export default function RagPanel() {
  ...
  async function handleUpload(e: React.FormEvent) {
    ...
    if (!isValidName(uploadName)) {
      ...
      setUploadError('Nome inválido. Use apenas letras, números, hífen (-) e underscore (_). Sem espaços ou acentos.')
      return
    }
    ...
    <div className="rag-panel">
      ...
      <section className="rag-card">
        ...
        <p className="rag-card__desc">
          Envie um arquivo <code>.txt</code> ou <code>.pdf</code> para indexar no ChromaDB.
          Se o arquivo for idêntico à versão atual, o reprocessamento é ignorado automaticamente.
        </p>
        ...
        <form className="rag-form" onSubmit={handleUpload}>
          <div className="rag-form__row">
            ...
            <input
              ...
              placeholder="ex: terms-of-service"
              value={uploadName}
            ...
            {uploadName && !isValidName(uploadName) && (
              <span className="rag-field-error">
                Use apenas letras, números, hífen (-) e underscore (_). Sem espaços ou acentos.
              </span>
            )}
          </div>
        ...
        {uploadState === 'success' && uploadResult && (
          <div className={`rag-feedback rag-feedback--${uploadResult.action === 'unchanged' ? 'warning' : 'success'}`}>
            {uploadResult.action === 'unchanged' ? (
              <>Arquivo idêntico à versão atual — nenhuma alteração necessária.</>
            ) : (
              <>
                  ...
                  <>
                    <tr key={doc.name} className="rag-table__row">
                      ...
                      <td className="rag-table__actions">
                        ...
                        >
                          {versionsLoading[doc.name] ? '…' : expandedVersions[doc.name] ? 'Ocultar' : 'Histórico'}
                        </button>
                        <button
                      ...
                      <tr key={`${doc.name}-versions`} className="rag-table__versions-row">
                        <td colSpan={8}>
                          <div className="rag-versions">
                            <strong>Histórico de versões — {doc.name}</strong>
