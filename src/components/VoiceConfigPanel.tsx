...
const VOICES: { id: VoiceOption; label: string; description: string }[] = [
  ...
  { id: 'fable',   label: 'Fable',   description: 'Expressiva, sotaque britânico' },
  { id: 'onyx',    label: 'Onyx',    description: 'Masculina, grave' },
  { id: 'nova',    label: 'Nova',    description: 'Feminina, amigável' },
  { id: 'shimmer', label: 'Shimmer', description: 'Feminina, suave' },
]

const FORMATS: { id: AudioFormat; label: string; description: string }[] = [
  { id: 'mp3',  label: 'MP3',  description: 'Compatível com todos os browsers' },
  { id: 'opus', label: 'Opus', description: 'Menor tamanho, ideal para streaming' },
  { id: 'aac',  label: 'AAC',  description: 'Boa compressão, ideal para iOS/macOS' },
  { id: 'flac', label: 'FLAC', description: 'Sem perda, maior fidelidade' },
  { id: 'wav',  label: 'WAV',  description: 'Sem compressão' },
]

const PREVIEW_TEXT = 'Olá! Sou o assistente da Funnair. Como posso ajudar com sua reserva hoje?'
  ...
  return (
    <div className="voice-config">
      <div className="voice-config__header">
        <div>
          <h2 className="voice-config__title">Configuração de Voz</h2>
        ...
        <div className="voice-config__header-actions">
          <button className="vc-btn vc-btn--ghost" onClick={handleReset}>Restaurar padrões</button>
          <button
          ...
          >
            {isTesting ? (
              ...
              <><svg width="15" height="15" viewBox="0 0 24 24" fill="currentColor"><path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02z"/></svg> Testar configuração</>
            )}
          </button>
          <button
      ...
      {/* Instruções */}
      <section className="vc-section">
        <h3 className="vc-section__title">Instruções de estilo
          <span className="vc-section__badge">gpt-4o-mini-tts-2025-03-20</span>
        </h3>
        <p className="vc-section__hint">
          Descreva o tom, personalidade e estilo de fala. Deixe em branco para comportamento padrão.
        </p>
        <textarea
          className="vc-instructions"
          rows={4}
          placeholder="Ex: Fale de forma animada e entusiasmada, como um atendente de companhia aérea muito empolgado com viagens. Use um tom caloroso e acolhedor."
          value={config.instructions}
          onChange={(e) => set('instructions', e.target.value)}
        />
        <div className="vc-instructions-presets">
          <span className="vc-instructions-presets__label">Sugestões:</span>
          {[
            { label: 'Formal', text: 'Fale de forma formal e profissional, como um atendente de companhia aérea de primeira classe.' },
            { label: 'Animado', text: 'Fale de forma animada e entusiasmada, com energia positiva e tom acolhedor.' },
            { label: 'Calmo', text: 'Fale de forma calma, pausada e tranquilizadora, como um piloto experiente.' },
            { label: 'Britânico', text: 'Speak with a British accent, formal and elegant, as a premium airline attendant.' },
          ].map((p) => (
      ...
      {/* Formato */}
      <section className="vc-section">
        <h3 className="vc-section__title">Formato de áudio</h3>
